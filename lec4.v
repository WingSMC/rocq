...
Lemma currying_1: forall A B C : Typ, exists t, ⊢ t [:] ((A ⊃ (B ⊃ C)) ⊃ ((A ⊃ B) ⊃ (A ⊃ C))).
Proof.
  intros.
  exists (lam (A ⊃ (B ⊃ C)) (lam (A ⊃ B) (lam A (app (app (hyp 2) (hyp 0)) (app (hyp 1) (hyp 0)))))).
  apply STT_lam.
  apply STT_lam.
  apply STT_lam.
  apply STT_app with (A:=B ⊃ C).
  apply STT_app with (A:=A).
  apply STT_hypS.
  apply STT_hypS.
...
2:
  apply STT_hyp0.
  (* apply STT_app with (A:=hyp B ⊃ hyp C). *)
